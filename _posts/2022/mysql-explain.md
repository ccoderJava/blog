...
最近在梳理MySQL常用知识点，将已有知识点进行结构化、方便后续回顾巩固。

今天就来总结梳理一下常用的`explain`命令。

`explain`命令是用来获取查询执行计划的信息，可以了解查询优化器是如何决定执行查询的。
...
- `possible_keys`提示哪一个索引能够有助于高效的查询。
- `key`表示优化器采用哪一个索引可以最小化查询成本。
...
上面整理了explain一些常见的参数及其说明，但是实际进行SQL优化时，情况相对复杂，需要了解这些参数含义，明白当前SQL可以从哪些方面进行优化，索引优化、索引覆盖、是否回表，联表查询是否增加缓存buffer从而减少join次数等。
